...
if [[ "${#outputs[@]}" == "1" ]]; then
  ...
  sed -i "s/set \$scr1.*/set \$scr1          ${outputs[0]}/g" $HOME/.config/sway/config.d/environments
  sed -i "s/set \$scr2.*/set \$scr2          ${outputs[1]}/g" $HOME/.config/sway/config.d/environments
  sed -i "s/output \$scr1 pos.*/output \$scr1 pos           0     $(swaymsg -t get_outputs | jq -r '.[1].rect.height')/g" $HOME/.config/sway/config.d/outputs
  for a in $(seq 1 8); do
    if [[ $a -le 4 ]]; then
      sed -i "s/\"$a\".*/\"$a\"                    : [\"${outputs[0]}\"],/g" $HOME/.config/waybar/config
    else 
